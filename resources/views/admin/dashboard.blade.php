...
@section('content')
    <!-- Statistics Grid Cards (Adaptable Warm Cream & Gold High Contrast Cards) -->
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
        ...
        <!-- Foto Galeri Card -->
        <div class="admin-card-color admin-text-color p-6 rounded-2xl shadow-xl border-2 border-[#d4af37] flex items-center justify-between hover:scale-105 transition-all">
            <div>
                <p class="text-xs uppercase font-extrabold tracking-wider mb-1 admin-accent-color">Foto Galeri</p>
                <h3 class="font-headline-md text-5xl font-black text-[#b8860b]">{{ $galleriesCount }}</h3>
            </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-10">
        <!-- Welcome Banner Card -->
        <div class="admin-card-alt-color admin-text-color border-2 border-[#d4af37] p-8 rounded-2xl shadow-xl flex flex-col justify-between">
            <div>
                ...
                <h3 class="font-headline-md text-3xl font-black admin-accent-color mb-3">Selamat Bekerja!</h3>
                <p class="admin-muted-color text-sm leading-relaxed font-medium mb-6">
            ...
            <div>
                <a href="{{ route('admin.profile') }}" class="inline-flex items-center gap-2 bg-[#f2ca50] text-[#3c2f00] hover:brightness-110 px-6 py-3.5 rounded-xl font-extrabold text-sm shadow-lg transition-all">
        ...
        <!-- Cloud System Status -->
        <div class="admin-card-color border-2 border-[#d4af37] p-8 rounded-2xl shadow-xl">
            <h3 class="font-headline-md text-2xl font-black admin-accent-color mb-6 flex items-center gap-3">
            ...
            <div class="space-y-4 text-sm font-medium">
                <div class="flex justify-between items-center pb-3 border-b admin-border-color">
                    ...
                    <span class="font-bold text-emerald-600 flex items-center gap-1.5 bg-emerald-500/10 px-3 py-1 rounded-lg border border-emerald-500/30"><span class="w-2.5 h-2.5 rounded-full bg-emerald-500 animate-pulse"></span> Vercel Serverless</span>
                </div>
                <div class="flex justify-between items-center pb-3 border-b admin-border-color">
                    <span class="admin-muted-color">Database Engine:</span>
                    <span class="font-bold text-emerald-600 flex items-center gap-1.5 bg-emerald-500/10 px-3 py-1 rounded-lg border border-emerald-500/30"><span class="w-2.5 h-2.5 rounded-full bg-emerald-500 animate-pulse"></span> Supabase PostgreSQL</span>
                </div>
                <div class="flex justify-between items-center">
                    <span class="admin-muted-color">Penyimpanan Foto (Bucket):</span>
                    <span class="font-bold text-emerald-600 flex items-center gap-1.5 bg-emerald-500/10 px-3 py-1 rounded-lg border border-emerald-500/30"><span class="w-2.5 h-2.5 rounded-full bg-emerald-500 animate-pulse"></span> Supabase Storage S3</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="admin-card-color border-2 border-[#d4af37] rounded-2xl p-8 shadow-xl">
